...
@WebServlet("/ThematicDServlet")
public class ThematicDServlet extends BaseServlet {

    //String rootDirPath = "C:\\ftp\\ChangJiang\\专题数据图片"; 专题数据图片根目录
...
//                    String rootUrl = "http://202.114.194.19:8181/专题数据图片";
                    ...
                    String rootUrl = "http://202.114.194.19:8080/CJData/专题数据图片";
    ...
    public void deletePictures(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        NewsService service = new NewsService();
        String rootPath = "C:\\ChangJiang\\apache-tomcat-9.0.41\\webapps\\CJData\\专题数据图片";
//        String rootPath = "D://ftp";
    ...
    public void addData(HttpServletRequest req, HttpServletResponse resp) throws IOException, InvocationTargetException, IllegalAccessException {
        ...
        String result = "0";
        ThematicData thematicData = new ThematicData();
        ...
        BeanUtils.populate(thematicData, map);

        if (thematicData.getPublishTime() == null) {
            thematicData.setPublishTime(new Date());
        }

        ThematicService thematicService = new ThematicService();
        if (thematicService.dataAdd(thematicData)) {
            result = "1";
        }
        resp.setContentType("text/html;charset=utf-8");
        resp.setHeader("Access-Control-Allow-Origin", "*");//解决跨域问题，开发完毕时应该关闭
    ...
    public void listDataByKey(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (!map.containsKey("queryInfo")) {//全部查询
            result = thematicService.allData(currentPage, currentCount);
        } else {
            ...
            if (key.isEmpty()) {
                ...
                result = thematicService.dataByKey(key, value, currentPage, currentCount);
            }
        }
    ...
    public void getDataById(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        ThematicData data = new ThematicQuery().queryDataById(id);
        if (data != null) {
    ...
    public void updateData(HttpServletRequest req, HttpServletResponse resp) throws IOException, InvocationTargetException, IllegalAccessException {
        ...
        BeanUtils.populate(thematicData, map);
        ThematicService thematicService = new ThematicService();
        if (thematicService.dataUpdate(thematicData)) {
            result = "1";
        }
        resp.setContentType("text/html;charset=utf-8");
        resp.setHeader("Access-Control-Allow-Origin", "*");//解决跨域问题，开发完毕时应该关闭
    ...
    public void deleteData(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        ThematicService thematicService = new ThematicService();
        if (thematicService.dataDelete(id)) {
            result = "1";
        }
